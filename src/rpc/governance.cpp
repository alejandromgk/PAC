...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 PAC irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on paccoincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in paccoin.conf\n"
                "\nArguments:\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* PAC features */
    { "paccoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "paccoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "paccoin",               "gobject",                &gobject,                true,  {} },
    { "paccoin",               "voteraw",                &voteraw,                true,  {} },
