...
const char * const BITCOIN_CONF_FILENAME = "paccoin.conf";
const char * const BITCOIN_PID_FILENAME = "paccoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "dash" is a composite category enabling all PAC-related debug output
                if(ptrCategory->count(std::string("paccoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PaccoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PaccoinCore
    // Mac: ~/Library/Application Support/PaccoinCore
    // Unix: ~/.paccoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PaccoinCore";
#else
    fs::path pathRet;
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty paccoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override paccoin.conf
