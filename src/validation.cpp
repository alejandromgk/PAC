...
#if defined(NDEBUG)
# error "Paccoin Core cannot be compiled without assertions."
#endif
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    if(pindexNew->nChainDelay != 0) {
        LogPrintf("%s: Block belong to a chain under punishment - Delay value: %i, BlockHeight: %d\n",__func__, pindexNew->nChainDelay,pindexNew->nHeight);
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(PAC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(PAC): spork is off, skipping transaction locking checks\n");
    }
