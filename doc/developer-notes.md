...
PAC Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 7112.
...
However, in open source software such as PAC Core, where everyone uses
...
pay attention to for reviewers of PAC Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: PAC Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against PAC Core so that they can be integrated
